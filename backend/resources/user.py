...
class UserAbout(Resource):
    parser = reqparse.RequestParser()
    parser.add_argument("about", type=str, required=False, help="This field cannot be left blank!")
    @classmethod
    @jwt_required
    def get(cls):
        id = get_jwt_identity()
    ...
    def put(cls):
            ...
            if not data.get('about'):
                return {'message': "Brak danych"}, 404
            user.about = data['about']
            user.save_to_db()
            return {'message': 'Ustawiono nową wartość about'}, 200
